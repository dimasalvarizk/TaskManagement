...
export const EmailPreviewModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-4xl bg-white dark:bg-[#16181f] border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl overflow-hidden flex h-[560px] text-xs select-none">
        {/* Sent Emails Outbox List */}
        <div className="w-72 border-r border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-[#12141a] p-4 space-y-3 shrink-0 flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-1.5 overflow-y-auto max-h-[440px]">
              {sentEmails.map((em) => (
                <button
                  key={em.id}
                  onClick={() => setActiveEmailPreview(em)}
        ...
        {/* Live Email HTML Inspector */}
        <div className="flex-1 flex flex-col justify-between overflow-hidden bg-slate-100 dark:bg-[#0f1117]">
          {/* Header */}
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-[#16181f]">
            <div>
          ...
          <div className="flex-1 p-5 overflow-y-auto">
            {currentEmail ? (
              <div
                className="w-full h-full rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-xs overflow-auto"
